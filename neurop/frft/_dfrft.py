...
def idfrft(x: torch.Tensor, alpha: Union[float, torch.Tensor], *, dim: int = -1) -> torch.Tensor:
    ...
    Arguments:
        ...
        alpha (Union[float, torch.Tensor]): Fractional order of the transform.
        dim (int): Dimension along which to perform the inverse transform.
...
def dfrft(x: torch.Tensor, alpha: Union[float, torch.Tensor], *, dim: int = -1) -> torch.Tensor:
    ...
    Arguments:
        x (torch.Tensor): Input tensor to be transformed.
        alpha (Union[float, torch.Tensor]): Fractional order of the transform.
        dim (int): Dimension along which to perform the transform.
...
def idfrftmtx(
    ...
    *,
    ...
    return dfrftmtx(N, -alpha, approx_order=approx_order, device=device)
...
def dfrftmtx(
    ...
    *,
    ...
    evals = torch.exp(-1j * alpha * (torch.pi / 2) * idx).type(torch.complex64)
