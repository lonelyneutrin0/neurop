...
from ..layers.skip_connections import Connection

import torch
import torch.nn as nn

from ..base import NeuralOperator, NeuralOperatorBuilder
from ..layers.spectral_convolution import SpectralConv, FFTNormType
...
class FourierOperator(NeuralOperator):
    ...
    def __init__(self,
                 ...
                 skip_connections: Union[Connection, List[Connection]] = Connection.IDENTITY,
                 n_kernel: int = -1,
                 use_feature_mlp: bool = True,
                 feature_mlp_module: Type[nn.Module] = ConvFeatureMLP,
                 feature_mlp_depth: int = 4,
                 feature_mlp_skip_connections: Union[Connection, List[Connection]] = Connection.IDENTITY,
...
class FourierOperatorBuilder(NeuralOperatorBuilder):
    ...
    def set_conv_module(self, 
        ...
        Arguments:
            ...
            skip_connection (Connection): Skip connection to use in the FourierOperator.
            conv_normalizer (Type[nn.Module]): Normalizer to use for the convolution layer.
    ...
    def set_feature_mlp(self, 
                        ...
                        feature_mlp_normalizer: Type[nn.Module],
                        ) -> Self:
        """
        ...
        Arguments:
            ...
            feature_mlp_depth (int): Depth of the feature MLP.
            feature_mlp_skip_connection (Connection): Skip connection to use in the feature MLP.
            feature_expansion_factor (float): Expansion factor for the feature MLP.
            feature_mlp_normalizer (Type[nn.Module]): Normalizer to use for the feature MLP.
